...
import { ArrowLeft, Camera, Droplets, CheckCircle, Save, Loader2, Send, Trash2, Brush, Lock, AlertCircle, DollarSign } from 'lucide-react';
...
const ServiceDetail: React.FC<ServiceDetailProps> = ({ task, onBack, onUpdate, fleet, adminChecklist }) => {
    ...
    const [collectedTips, setCollectedTips] = useState<number>(task?.collectedTips || 0);
    ...
    return (
        <div className="pb-32 max-w-3xl mx-auto px-4 animate-fadeIn">
            <div className="flex items-center mb-8 pt-4">
                <button onClick={() => { vibrate(10); onBack(); }} className="p-3 bg-white rounded-[24px] shadow-glass border border-brand-border text-brand-muted hover:text-brand-primary active:scale-95 transition-all"><ArrowLeft className="w-5 h-5" /></button>
                <div className="ml-4">
            ...
            <div className="space-y-6">
                ...
                {(isFuelLocked || isWaterLocked) && (
                    <div className="bg-brand-primary text-white p-8 rounded-[40px] shadow-spatial flex items-start gap-5 border-l-8 border-brand-gold animate-slideUp">
                ...
                {/* VERIFICAÇÃO TÉCNICA */}
                <div className="glass-card p-8 md:p-10 rounded-[40px] shadow-glass border border-brand-border/40 space-y-8">
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <button
                                ...
                                className={`p-6 rounded-[32px] border-2 shadow-sm flex flex-col items-center gap-3 transition-all active:scale-95 ${isFuelLocked || fuelPhotoTaken ? 'bg-green-50 border-green-500 text-green-700 shadow-md' : 'border-dashed border-brand-border text-brand-muted hover:border-brand-primary hover:text-brand-primary'}`}
                            >
                            ...
                            <button
                                ...
                                className={`p-6 rounded-[32px] border-2 shadow-sm flex flex-col items-center gap-3 transition-all active:scale-95 ${isWaterLocked || waterPhotoTaken ? 'bg-blue-50 border-blue-500 text-blue-700 shadow-md' : 'border-dashed border-brand-border text-brand-muted hover:border-brand-primary hover:text-brand-primary'}`}
                            >
                        ...
                        <div className="space-y-2">
                            {(adminChecklist || [
                                ...
                                <button
                                    ...
                                    className={`w-full p-5 rounded-[24px] border flex items-center justify-between transition-all active:scale-[0.98] ${completedSteps.includes(step.id) ? 'bg-brand-primary text-white border-brand-primary shadow-spatial' : 'bg-white text-brand-muted border-brand-border shadow-sm hover:border-brand-primary/50'}`}
                                >
                    ...
                    <div className="space-y-4 pt-4 border-t border-brand-border/40">
                        <label className="text-sm font-bold text-brand-dark uppercase tracking-widest flex items-center gap-2">
                            <DollarSign className="w-5 h-5 text-brand-gold" /> Registo de Gorjetas (€)
                        </label>
                        <div className="relative">
                            <span className="absolute left-6 top-1/2 -translate-y-1/2 text-brand-muted font-bold text-lg">€</span>
                            <input
                                type="number"
                                min="0"
                                value={collectedTips}
                                onChange={(e) => setCollectedTips(Number(e.target.value) || 0)}
                                className="w-full bg-brand-bg border-none pl-14 pr-6 py-5 rounded-[24px] text-lg font-bold text-brand-dark focus:ring-2 focus:ring-brand-gold outline-none shadow-inner"
                                placeholder="0"
                            />
                        </div>
                        <p className="text-[10px] text-brand-muted uppercase font-bold tracking-widest">Valor recebido será refletido no ecrã de ganhos.</p>
                    </div>
                </div>
                ...
                <button
                    ...
                    className="w-full py-6 bg-brand-primary text-white rounded-full font-black uppercase tracking-[0.2em] shadow-spatial hover:-translate-y-1 active:scale-95 transition-all duration-300 disabled:opacity-30 disabled:grayscale"
                >
